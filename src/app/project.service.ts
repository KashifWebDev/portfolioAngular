...
export class ProjectService {
  ...
  badgeList: string[] = ['success', 'danger', 'warning', 'info', 'primary', 'secondary'];
  ...
  findProject(id: number): project{
    return this.projects.filter((project: any) => { return  project.id == id })[0];
  }
  ...
  getRandomBadge(index: number): string {
    const randomIndex = Math.floor(Math.random() * this.badgeList.length);
    return 'badge-' + this.badgeList[randomIndex];
  }
